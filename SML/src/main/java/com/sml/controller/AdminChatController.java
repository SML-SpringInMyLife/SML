...
public class AdminChatController {
	...
	private void handleNewConnection(Session session, MemberVO member) {
		...
		if (adminCk == 0) { // 일반 사용자
			...
			responseMessage.put("type", "info");
			responseMessage.put("userId", "알림");
			responseMessage.put("memName", "메시지");
			responseMessage.put("content", "채팅방이 열렸습니다.");
			...
			UserSessionInfo sessionInfo = new UserSessionInfo(userId, session, member.getMemName(), member.getMemCode(),
					conversationId);
	...
	private void joinChatRoom(String conversationId, Session session) throws IOException {
        ...
        if (userId == null) {
            ...
            errorMessage.put("userId", "알림");
            errorMessage.put("memName", "메시지");
        ...
        if (sessionInfo != null) {
            ...
            joinMessage.put("userId", "알림");
            joinMessage.put("memName", "메시지");
            joinMessage.put("content", userId + "(" + conversationId + ") 님의 채팅방에 입장했습니다");
            ...
            if (buffer != null) {
                ...
                bufferMessage.put("userId", "알림");
                bufferMessage.put("memName", "메시지");
                bufferMessage.put("timestamp", System.currentTimeMillis());
	...
	private void processChatMessage(JSONObject jsonMessage, MemberVO member) throws IOException {
		String userId = member.getMemId() != null ? member.getMemId() : "알림";
		String memName = member.getMemName() != null ? member.getMemName() : "메시지";
	...
	private void sendWelcomeMessage(Session session, String userId) throws IOException {
		...
		welcomeMessage.put("userId", "알림");
		welcomeMessage.put("memName", "메시지");
		welcomeMessage.put("content", "안녕하세요, " + userId + "님! 잠시만 기다려주시면 상담사가 입장합니다.");
	...
	private void notifyAdmins(String userId) throws IOException {
		for (Session adminSession : adminSessions.values()) {
			if (adminSession.isOpen()) {
				...
				notifyMessage.put("userId", "알림");
				notifyMessage.put("memName", "메시지");
				notifyMessage.put("content", "상담을 종료했습니다.");
