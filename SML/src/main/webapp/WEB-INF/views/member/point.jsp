	...
	<main>
		<div class="member-container">
			...
			<div class="member-main-content">
				...
				<form action="${appServlet}/member/point" method="get"
					...
					<select id="selectbox" name="selectDate">
						...
						<c:forEach begin="${currentYear - 0}" end="${currentYear}"
							var="year">
							<c:forEach begin="1" end="12" var="month">
								<c:if
									test="${year < currentYear || (year == currentYear && month <= currentMonth)}">
				...
				<table class="course-table">
					...
					<tbody id="courseList">
						...
						<tr class="course-group">
							<td colspan="8" data-label="포인트">현재 포인트 : <c:out
									value="${ TotalPoint }" /> 점 입니다.
							</td>
						</tr>
						<c:forEach items="${list}" var="list" varStatus="status">
							<tr>
								...
								<td><fmt:formatDate value="${list.pointDate}"
										pattern="yyyy-MM-dd HH:mm" /></td>
								<td><c:out value="${ list.pointComment }" /></td>
								<td><fmt:formatNumber value="${ list.pointPrice}"
										pattern="#,###" />P</td>
								<td><c:choose>
				...
				<div class="pageMaker_wrap">
					<ul class="pageMaker">
						...
						<c:if test="${pageInfo.prev}">
							...
							<a href="${appServlet}/member/mycourses?pageNum=${pageInfo.pageStart - 1}">이전</a>
							</li>
						</c:if>
						<!-- 페이지 번호 -->
						<c:forEach begin="${pageInfo.pageStart}" end="${pageInfo.pageEnd}"
							var="num">
							<li
								class="pageMaker_btn ${pageInfo.cri.pageNum == num ? 'active' : ''}">

								<a href="${appServlet}/member/mycourses?pageNum=${num}">${num}</a>
							</li>
						</c:forEach>
						<!-- 다음 버튼 -->
						<c:if test="${pageInfo.next}">
							<li class="pageMaker_btn next">
							<a href="${appServlet}/member/mycourses?pageNum=${pageInfo.pageEnd + 1}">다음</a>
							</li>
						</c:if>
					</ul>
				</div>
