...
- **BCrypt를 통한 비밀번호 암호화 및 세션 관리**: 사용자(회원)가 입력한 비밀번호를 BCrypt로 암호화하여 저장하고 로그인 시 암호화된 비밀번호와 비교하여 검증합니다. 로그인 성공 시 세션을 생성하여 로그인 상태를 유지하고, 비밀번호는 세션에 저장되지 않아 사용자의 민감한 정보가 보호됩니다.

- **이메일 인증 및 회원정보 수정**: 회원가입 시 이메일로 인증번호를 전송하여 사용자가 올바른 이메일을 입력했는지 확인합니다. 사용자는 마이페이지에서 자신의 회원 정보를 수정할 수 있습니다.

- **포인트 관리 및 출석체크 기능**: 사용자는 마이페이지를 통해 특정 기간 동안 적립된 포인트를 조회하고 매일 출석 기록을 갱신할 수 있습니다. 출석 체크 기록은 데이터베이스에 저장되며 이후 사용자 확인 및 분석에 활용됩니다.
...
- **공지사항 등록 수정 및 상세 조회**: 관리자는 새로운 공지사항을 등록 수정 삭제 할 수 있습니다. 모든 로그인 사용자는 공지사항을 카테고리별로 검색 및 조회 할 수 있습니다.

- **조회수 및 좋아요**: 사용자는 공지사항 상세 페이지 상단에 표시 된 ♥ 표시를 클릭하여 공지사항에 반응할 수 있습니다. 사용자가 반응한 좋아요는 곧바로 공지사항에 반영되고 글을 클릭하는 수 만큼 조회수가 반영됩니다.
...
- **게시판 기능과 회원 기능 상호작용**: 사용자는 커뮤니티 게시판에서 게시글을 작성, 수정, 삭제할 수 있습니다. 각 게시글에는 조회수와 좋아요 기능이 포함되어 있어 사용자가 편리하게 자신의 의견을 표현할 수 있습니다. 사용자가 원하면 하단의 비밀글 체크박스를 통하여 비밀글 설정을 할 수 있습니다. 글 작성시 회원 포인트가 적립되며 마이페지에서 확인할 수 있습니다.

- **댓글 기능**: 게시글에는 댓글을 작성할 수 있고 이를 통해 사용자 간의 소통이 이루어집니다. 작성된 댓글은 게시글 아래에 표시되며, 댓글 작성자는 댓글을 수정하거나 삭제할 수 있습니다.

- **수강신청 및 강사 관리**: 사용자는 수강신청 목록을 확인하고 수강신청 할 수 있습니다. 관리자는 강사와 강의를 등록하고 관리할 수 있습니다. 관리자는 강사 정보와 수업 등록 정보를 수정 및 삭제할 수 있으며, 강사의 상세 정보를 조회하고 수정할 수 있습니다. 사용자가 강의를 수강신청하고 신청이 완료되면 회원 포인트가 적립되며 마이페이지에서 확인할 수 있습니다.
...
- **SMS 발송 및 관리**: 관리자는 특정 회원에게 SMS를 발송하고 발송 결과를 실시간으로 확인할 수 있습니다. 관리자가 보낸 SMS는 관리자 페이지 'SMS 관리'에서 확인 가능하며 상세 조회 할 수 있습니다.

- **실시간 채팅 관리 및 기록**: WebSocket을 사용하여 사용자와 실시간으로 채팅을 제공하며 채팅 메시지는 JSON 형식으로 처리됩니다. 채팅 세션 종료 시 채팅 내용이 데이터베이스에 저장되어 추후 대화 기록을 조회할 수 있습니다. 관리자페이지 '채팅상담 관리'를 통해 채팅상담 기록을 확인할 수 있습니다.
...
- **후원 결제**: 사용자는 '후원 하기' 버튼으로 후원을 할 수 있습니다. 카카오페이 QR 코드로 연결되며, 결제 후 결제 정보가 DB에 저장되고 관리자 계정으로 로그인 시 후원 내역 페이지에서 결제 상세 내역을 확인할 수 있습니다.
...
- **서비스 레이어 테스트**: JUnit을 사용하여 서비스 계층의 비즈니스 로직을 단위별로 테스트하였습니다. 각 메서드의 예상 동작을 검증하여 오류를 보다 쉽게 찾고 해결할 수 있었습니다.
...
- **RESTful API 설계 및 구현**: Kakao Maps API를 통해 사용자의 현재 위치 기반 장소 검색 기능을 구현하였으며, IamPort API를 통해 간편 결제 시스템을 구성하였고, ckEditor API를 이용해 API에서 제공되는 게시판 글쓰기 기능을 사용하였습니다. CoolSMS API를 통해 관리자가 사용자에게 문자를 보낼 수 있는 기능을 구현하였고, Daum Postcode API로 사용자에게 편리한 주소 찾기 서비스를 제공합니다.

- **API 문서화**: 특정 API key들을 apikey.properties 에 추가하여 코드를 분리하였고 key 정보가 유출되지 않도록 보안을 강화하였습니다.
